...
const Habit = memo(({data, canSubtract, updateSelectedAmount = null}) => {
    ...
    const {id, name, completion, setting, repeat, type, label, color, referenceGoal, location, date, completed} = data
    ...
    useEffect(() => {
        const fetchHabitData = async () => {
            ...
            if (data == null) {
                const periodData = await historyRepo.getPeriodData(id, selectedDate) ?? null
                const previousStreak = await historyRepo.getStrictPreviousPeriodStreak(id, selectedDate)
                ...
                if (periodData) {
                    ...
                    setCurrentStreak(periodData.streak == 0 && previousStreak > 0 ? previousStreak : periodData.streak) // was periodData.streak == 0 ? periodData.streak : previousStreak but that caused weekly dates that didn't have log save to show 0 for streak instead of correct
                }
                else {
                ...
                /*try { // EDIT change the logic here so it is based on completion or not
                const previousStreak = await historyRepo.getPreviousPeriodStreak(id, selectedDate) ?? 0
                ...
                if (data.periodStreak == 0) {
                    const previousStreak = await historyRepo.getStrictPreviousPeriodStreak(id, selectedDate)
                    setCurrentStreak(previousStreak > 0 ? previousStreak : data.periodStreak)
                }
                else {
    ...
    async function updateHabitCompletion(amount, historyData) {
        const periodKey = historyData ? historyData.periodKey : dynamicData.periodKey
        ...
        try {
        await historyRepo.setCompletion(id, amount, selectedDate, goal, periodKey, currentStreak)
    ...
    useEffect(() => {
        const setStreaks = async () => {
            ...
            catch (err) {console.error("error setting the streak in Habit.jsx", err, "from src:", src)}
            //}
        }
        ...
        const inner = async () => {
            if (dynamicData == null && amount > 0) {
                ...
                const historyData = await historyRepo.getEntryWithCheck(id, selectedDate)
            ...
            console.log("Completed:", isCompleted, "PeriodAmount:", periodAmount, "Goal:", goal)
    ...
    const HabitButton = ({incrementor}) => {
        return (
            <TouchableOpacity onPress={() => {addMetric(incrementor)}} className={`w-16 h-16 overflow-hidden rounded-2xl justify-center items-center ${(incrementor < 0 && amount <= 0) || periodAmount < goal ? "bg-background-80" : "bg-highlight-60"}`} disabled={incrementor < 0 && amount <= 0}>
                {
                    isLoading ? (
                        <ActivityIndicator size="large" />
                    ) : (
                        <Image
                            source={incrementor > 0 ? (periodAmount < goal ? icons.addBox : icons.check) : icons.subtractLine}
                            resizeMode='cover'
                            className="w-[3rem] h-[3rem]"
                            tintColor={(incrementor < 0 && amount <= 0) || periodAmount < goal ? tailwindColors['highlight']['80'] : tailwindColors['background']['90']}
                        />
                    )
                }
            </TouchableOpacity>  
    ...
    const HabitCompletionDisplay = () => {
        return (
            <Text className="text-highlight-80 text-lg font-generalsans-medium"> {/* EDIT remove text-xs*/}
